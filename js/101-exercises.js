...
function secondToLast(array) {
    if (typeof array === "string") {
        ...
        var newArrReverse = array.reverse();
    }
...
function onlyPositiveEvens(input) {
    var posEven = input.filter(n => n % 2 === 0 && n > 0);
    return posEven;
}
...
function onlyPositiveOdds(input) {
    var posOdd = input.filter(n => n % 2 !== 0 && n > 0);
    return posOdd;
}
...
function elementsTimesTwo(ary) {
    var doubleEachItem = ary.map(x => x * 2);
    return doubleEachItem;
}
...
function getYearPublished(obj) {
    var yearPublished = obj.year_published;
